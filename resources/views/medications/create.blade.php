<x-app-layout>
    <div class="flex justify-center">
        <div class="w-full max-w-2xl bg-white shadow rounded-lg p-6 mt-8">
            <h1 class="text-2xl font-bold text-center mb-6">薬を登録</h1>
            ...
            <form method="POST" action="{{ route('medications.store') }}" class="space-y-5">
                @csrf
                ...
                {{-- 用量 --}}
                <div>
                    <label class="block font-medium mb-1">用量 <span class="text-red-600">*</span></label>
                ...
                {{-- 効果 --}}
                <div>
                    <label class="block font-medium mb-1">効果 <span class="text-red-600">*</span></label>
                    <textarea name="effects" rows="3"
                            class="w-full border-gray-300 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500"
                            required>{{ old('effects') }}</textarea>
                    @error('effects')
                        <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                {{-- 副作用 --}}
                <div>
                    <label class="block font-medium mb-1">副作用 <span class="text-red-600">*</span></label>
