<x-app-layout>
  <x-slot name="header">
    <h2 class="text-xl font-semibold">記録の編集</h2>
  </x-slot>
    ...
    <form method="POST" action="{{ route('records.update', $record) }}" class="space-y-6">
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        <div>
          ...
          <input type="date" name="taken_date"
                 value="{{ old('taken_date', $record->taken_at->toDateString()) }}"
      ...
      <template id="reason-template">
        <div class="mt-3">
            ...
            <select name="medications[0][reason_not_taken]"
                    data-name="reason_not_taken"
                    class="w-full border rounded px-3 py-2">
              <option value="">選択してください</option>
  ...
  {{-- PHPからJSONに変換したデータをJavaScript変数として定義 --}}
  <script>
